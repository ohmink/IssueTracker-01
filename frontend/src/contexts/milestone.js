...
const MilestoneProvider = ({ children }) => {
  ...
  useEffect(() => {
    const getMilestones = async () => {
      try {
        ...
        const result = await axios.get('http://49.50.166.157:3000/api/milestone', { withCredentials: true });
  ...
  return (
    <MilestoneStateContext.Provider value={state}>
      <MilestoneDispatchContext.Provider value={dispatch}>{children}</MilestoneDispatchContext.Provider>
